...
- **`PERMISSION_KEY_PATTERN.md`/`PERMISSION_ROUTER_IMPLEMENTATION.md` are missing from `origin/main`** (confirmed via `git cat-file -e origin/main:<path>` → not found). Deleted by PR #1146 inside an unrelated squashed commit ("chore: remove documentation files", part of "feat(ci): add PR preview infrastructure") — looks accidental. They still exist locally on `kirylDev` (branched before that deletion) and are what 02/04/05 in this KB reference; a fresh clone of `origin/main` won't have them. Worth a small upstream PR to restore.
