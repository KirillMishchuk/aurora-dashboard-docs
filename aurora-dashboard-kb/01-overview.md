...
├── packages/
│   ├── aurora/                # @cobaltcore-dev/aurora v1.3.x — THE library (server + client), published to npm
...
- Repo root `pnpm-workspace.yaml` sets `minimumReleaseAge: 10080` (dependencies must be ≥7 days old, npm supply-chain guard), with `minimumReleaseAgeExclude` listing `tmp`, `@cloudoperators/juno-ui-components` and `@fastify/static@10.1.2`. **The long CVE-pinning block that used to sit beside it is gone (#1267, merged 03.09.2026):** ~20 `overrides` entries plus their exclude lines (js-yaml, brace-expansion, shell-quote, fast-uri, find-my-way, postcss, lodash, esbuild, nanoid, tmp, `@fastify/static`) were deleted once the real dependency tree resolved high enough on its own; the single survivor is `"@fastify/static@<10.1.2": ^10.1.2`, kept because that package arrives transitively via `@fastify/vite` and can't be bumped directly. Don't re-add an override for something the tree already satisfies — the file is deliberately kept minimal. **A second override was added the next day (#1269, merged 04.09.2026):** `"tmp@<0.2.6": ">=0.2.6"`, a path-traversal CVE fix for the transitive `tmp@0.0.33` pulled in via `cz-customizable → inquirer → external-editor`. Its paired changeset (`.changeset/fix-tmp-vulnerability.md`) was deleted three hours later by #1276 ("remove bad changeset", no reason given in the commit) — the lockfile/override change itself was not reverted, only the changeset, so this fix won't show up in a `@cobaltcore-dev/aurora-dashboard` CHANGELOG entry unless someone adds a fresh changeset before the next release. **A third override followed two weeks later (#1315, merged 18.09.2026):** `"js-yaml@>=3.0.0 <3.15.2": "3.15.2"` and `"js-yaml@>=4.0.0 <4.3.2": "4.3.2"` (CPU-DoS via empty merge keys) plus `"esbuild@>=0.27.3 <0.28.1": ">=0.28.1"` (dev-server arbitrary file read); `vitest` bumped `^4.1.10`→`^4.1.11` in `aurora`/`policy-engine`/`signal-openstack` (`@vitest/mocker` path traversal) via direct `package.json` edits, not an override. `pnpm audit` reported clean afterward. **Same changeset-loss pattern as the #1269/#1276 case above:** a changeset was added and then deleted within the same squashed commit ("chore(deps): add changeset for security patch" → "Delete .changeset/patch-security-advisories.md"), so this fix also carries no changeset — won't appear in the CHANGELOG unless one is added before the next release.
- **`@cloudoperators/juno-ui-components` is a real npm dependency, not a pnpm-workspace link.** There is no local build-time coupling to the sibling `../juno` clone that lives in this SAP workspace folder (`cloudoperators/juno`, the design system's own monorepo) — that clone is purely a reference copy for reading actual component source/props when the published package's types/docs aren't enough. It tracks `origin/main`, which normally runs ahead of the version pinned here — currently `9.4.0` (bumped from `9.1.0` for the `Status` component in #1209, merged 27.08.2026) — check out the git tag matching the pinned version (`@cloudoperators/juno-ui-components@<version>`) in `juno/` before trusting its source as authoritative for currently-running behavior.
...
- **Compute components moved to per-feature directories** (#1245, merged 01.09.2026): Flavors/Images components live under `compute/flavors/-components/` and `compute/images/-components/` now, not the old shared `compute/-components/Flavors|Images/`. The same PR deleted the long-dead `compute/-components/{Instances,KeyPairs,ServerGroups}/` component trees (List views for routes that were never actually wired up — see 05-domain-map.md "Compute — servers"/"Compute — keypairs"), unused since at least #1222.
- **Security dependency refresh, no package-version change (#1267, merged 03.09.2026).** `fastify` 5.8.5 → 5.12.1 in `apps/dashboard` (CVE-2026-3635, `trustProxy` spoofing), root `@commitlint/cli` + `@commitlint/config-conventional` `^20.5.3` → `^21.2.2`, and `fast-uri` to 3.1.6 / 4.1.3 (CVE-2026-13676 variants) through the lockfile. The same PR added `fastify: ^5.12.1` to `packages/aurora`'s **devDependencies** — fastify stays a `peerDependency` (`^5.0.0`) for consumers; the devDep only gives the package's own build/tests a concrete copy, so this is not a change to the dependency classification rule below. Queued changesets since the 01.09.2026 pin (including this one) were released the next day: **#1205 (merged 04.09.2026)** bumped `@cobaltcore-dev/aurora` 1.1.0 → **1.2.0** (minor — covers #1268's breaking service-extension rework plus Ceph lifecycle rules #1178 and ~20 other queued changesets) and `@cobaltcore-dev/dashboard` 1.2.11 → **1.2.12** (patch). **#1277 (merged 08.09.2026)** then bumped `@cobaltcore-dev/aurora` 1.2.0 → **1.3.0** (minor — #1263's floating-IP fixes) and `@cobaltcore-dev/dashboard` 1.2.12 → **1.2.13** (patch).
- **Stray `PR_SUMMARY.md` at repo root** (introduced by #1283, merged 08.09.2026, still present at HEAD): a PR-description-shaped scratch file (looks AI-agent-authored — Summary/Changes/Test Plan sections) that landed in the commit and was never removed. Not documentation, not referenced anywhere; safe to ignore, and a candidate for cleanup in a future PR.
