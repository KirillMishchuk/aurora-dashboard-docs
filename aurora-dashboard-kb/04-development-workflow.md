...
- **`.github/copilot-instructions.md`** drives GitHub Copilot's automated PR review (separate from the `.claude/` assets below, which are Claude Code-only and don't ship upstream). Originally just Part A (React 19/tRPC/TanStack/Juno component conventions + general review instructions); **#1290 (merged 11.09.2026)** roughly 4x'd the file by adding Part B — component library requirements, list-page zone structure, spacing rules, color tokens, modal structure, empty/loading/error states, data fetching, permissions/RBAC, URL state, toast notifications, form validation, file/component naming, import order, accessibility/i18n, DataGrid column/row interaction, Messages-vs-toast guidance, reusable-hooks pointers, PopupMenu item ordering, and UX writing standards, plus a "Quick Reject Checklist" — derived from the compute/storage routes' reference implementations. Worth reading before touching list/detail-page UI; note the section numbering has a couple of accidental duplicates (two `B.16`s, two `B.17`s) from the PR's own Copilot-suggested edits. **#1292** (merged 15.09.2026) followed up: added a Repository Context block, a three-pass review instruction, moved the Quick Reject Checklist to the top of the file, tagged every B.x section `[CRITICAL/STANDARD/STYLE]`, fixed the duplicate numbering (PopupMenu → `B.18`, UX Writing → `B.19`), added a Review Style note telling Copilot not to repeat prior findings, and trimmed several verbose sections (B.5, B.6, B.11, B.16, B.18, B.19). **#1309** (merged 17.09.2026, Andreas Pfau) followed up again: added a "Review Scope" section telling Copilot to skip checks already enforced by TypeScript/Prettier/Lingui (don't duplicate the toolchain), fixed B.19 title-case guidance, tightened B.5/B.16 (no redundant `Message` inside a modal that already shows one, ban `autoDismiss` on error toasts), added a B.6 HTTP status-code matrix (401/403/404/500 handling, full-page vs inline), a B.8 decision matrix for disabled-vs-hidden controls under missing permissions, B.15 bulk-action button rules (disabled until a row is selected, tri-state select-all), B.11 label-not-placeholder guidance, and corrected a B.18 divider-exception rule.
...
- **Server router test doubles**: build a fake context via a local `mockContext.ts` (e.g. `Storage/routers/ceph/mockContext.ts`) exposing `vi.fn()`-based `validateSession`/`rescopeSession`/`openstack.service(name)`, then call the router through `createCallerFactory(auroraRouter(...))`. External SDK clients (e.g. the S3 client) are mocked separately with `vi.mock(...)` and asserted on both return value and call count.
- **Global test setup** (`packages/aurora/vitest.setup.ts`): mocks `@tanstack/react-router` globally (a shared `mockRouter` with `invalidate`/`navigate`/`history.back` spies, so assertions on them work across tests) and `@/client/trpcClient` (components never hit a real client in unit tests), stubs SVG `?react` imports, `ResizeObserver`, and `localStorage`, and resets Lingui to `i18n.activate("en")` before each test.
- **React component tests**: wrap in `<I18nProvider i18n={i18n}>` only when the component actually renders Lingui-translated text (see `ContentHeader.test.tsx`, `LoginForm.test.tsx`) — don't add it reflexively to every test file.
- **signal-openstack tests**: stub `global.fetch` directly (e.g. `client.test.ts`) — this package has no HTTP-mocking dependency (no nock/msw).
- **policy-engine tests**: `integration.test.ts` deliberately keeps its `vi.mock(...)` calls commented out and exercises the real lexer/parser/evaluator against fixtures — the pattern to follow when adding rule-evaluation coverage, not leftover scaffolding to reactivate.
- **Debugging a permission denial**: `packages/policy-engine/src/debugTrace.ts` — use `PolicyEngine.checkRuleWithDebug`/`checkRuleWithTrace` rather than re-deriving the policy logic by hand.
