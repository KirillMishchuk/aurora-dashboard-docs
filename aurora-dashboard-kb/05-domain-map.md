...
| **Storage — Swift** | `Storage/routers/swift/swiftRouter.ts` | `storage/$provider/$storageType/…/objects` | `006_swift_object_storage_bff.md` (verified against Swift API; relative paths, SDK handles /v1/AUTH_) |
...
| **Permissions** | `policies/createPermissionRouter.ts` + per-domain `permissionRouter.ts` (Compute, Network, Storage) | UI checks `scope:resource:action` keys | `PERMISSION_KEY_PATTERN.md`, `PERMISSION_ROUTER_IMPLEMENTATION.md` (repo root — ⚠️ deleted from `origin/main` by PR #1146, accidental; still present locally on `kirylDev`, see 01-overview.md gotchas) |
