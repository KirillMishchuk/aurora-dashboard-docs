...
| **Storage — Swift** | `Storage/routers/swift/swiftRouter.ts` | `storage/$provider/$storageType/…/objects`; object downloads/previews run in a Web Worker off the main thread, at parity with Ceph (#1155, merged 2026-08-13), see 02-architecture "Cross-cutting mechanisms" and `prs/1155-…md` for findings shipped unfixed | `006_swift_object_storage_bff.md` (verified against Swift API; relative paths, SDK handles /v1/AUTH_) |
