...
1. **`ImageListView.tsx` — чекбокс "выбрать всё" в хедере `DataGrid` ломает мультистраничный выбор и показывает неверное состояние.** (confidence 98)
   ...
   Подтверждено независимо: живые комментарии Copilot и CodeRabbit на самом PR #1211 (`ImageListView.tsx:612/617/623`) указывают на то же — чекбокс в хедере вообще противоречит issue #1169, на который ссылается PR, и ломает мультистраничный выбор. Автор (TilmanHaupt) ответил на этот тред "no current behaviour is intended" — формулировка двусмысленная (может значить как "текущее поведение не задумано", так и обратное), поэтому не считаем вопрос закрытым решением автора; технически баг подтверждён построчным чтением кода независимо от трактовки комментария.
...
3. **`DeleteImageModal.tsx` не вызывает `markSubmitted()` — трекинг отправки удаления образа молча не срабатывает.** (confidence 80)
   ...
   Файл: `packages/aurora/src/client/routes/_auth/projects/$projectId/compute/-components/Images/-components/DeleteImageModal.tsx:36-48`.

**Также замечено (confidence 50-79, не набрало полной уверенности):**
- **[75]** `DeleteFlavorModal.tsx:87` — заголовок `t\`Delete Flavor "${flavorName}"\`` не защищён от `flavor === null` (пропс `flavor: Flavor | null`); при null `flavorName` — `undefined`, заголовок буквально показывает `Delete Flavor "undefined"`. Тело модалки такую защиту имеет (`{flavor && (...)}`), заголовок — нет. Живой комментарий Copilot на этом PR указывает то же самое, до сих пор не закрыт; реальная воспроизводимость ограничена узким окном (переходная анимация закрытия модалки, если `flavor` сбрасывается раньше, чем `isOpen` становится `false`), поэтому не поднято до headline-находки.
- **[70]** Swift Objects `DeleteObjectModal.tsx` — новый инпут подтверждения (добавлен именно этим PR, локальный `useState`, не через общий хук) использует `confirmText === "delete"` без `.trim()`, в отличие от самого хука `useDeleteConfirmation` (который через `.trim()` прощает случайные пробелы). Живой комментарий CodeRabbit на этом PR, не закрыт.
- **[65]** `EmptyContainerModal.tsx` — сообщение "Showing first N of total" опирается на потенциально устаревший `container.count` относительно жёсткого лимита показа в 100 объектов. Живой комментарий CodeRabbit, не закрыт.
...
---
Проанализировано: 27.08.2026 · коммит `0cb05e66`
