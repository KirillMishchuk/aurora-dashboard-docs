...
export const listSecurityGroupsInputSchema = projectScopedInputSchema.extend({
  ...
  // Filtered in the BFF layer, not forwarded to Neutron: `stateful` is not a confirmed filter on
  // GET /v2.0/security-groups, and on clouds running Neutron's filter validation an unsupported
  // query parameter fails the whole list request instead of being ignored.
  stateful: z.boolean().optional(),
