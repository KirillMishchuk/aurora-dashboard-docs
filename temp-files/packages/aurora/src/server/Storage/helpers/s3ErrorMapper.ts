...
export function mapS3ErrorToTRPCError(
  ...
  context: {
    ...
    bucket?: string
    key?: string
    // PreconditionFailed is S3's generic response for any failed conditional
    // request (If-Match, If-None-Match, etc). Only createFolder's
    // IfNoneMatch: "*" write actually means "already exists" — every other
    // caller sharing this mapper (versioning, policies, uploads, ordinary
    // object writes) would get a misleading CONFLICT/"already exists"
    // message for an unrelated precondition failure, so this must be opted
  ...
  if (errorCode === "AccessDenied") {
    ...
    parts.push(
      context.preconditionFailedMeansAlreadyExists ? "The object already exists" : "Precondition failed"
    )
