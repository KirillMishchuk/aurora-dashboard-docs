...
export function mapErrorResponseToTRPCError(
  ...
  context: {
    ...
    // 412 is a generic "precondition failed" status shared by conditional requests
    // (If-Match/If-None-Match/If-Modified-Since/If-Unmodified-Since on arbitrary
    // GET/HEAD/PUT calls, e.g. getObjectMetadata). Only the create-if-not-exists
    // callers (which send "If-None-Match: *" to detect an existing resource) know
    // that a 412 specifically means "already exists" — everyone else should get a
    // faithful PRECONDITION_FAILED instead of a misleading CONFLICT.
