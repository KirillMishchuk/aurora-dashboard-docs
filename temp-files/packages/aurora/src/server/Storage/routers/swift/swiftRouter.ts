  ...
  createContainer: projectScopedProcedure
    ...
    .mutation(async ({ input, ctx }): Promise<CreateContainerResult> => {
      return withErrorHandling(async () => {
        ...
        // The container already exists at this point (the PUT above returned 201), so rethrowing
        // here would make the client believe the create itself failed and potentially retry -
        // which would then hit the 202/CONFLICT branch above. Degrade instead: report the
        // container as created, but flag that its settings could not be applied.
    ...
    .mutation(async ({ input, ctx }): Promise<boolean> => {
      return withErrorHandling(async () => {
        ...
        await swift.put(url, new ArrayBuffer(0), { headers }).catch((error) => {
          // 412 (from If-None-Match: "*" above) maps to CONFLICT via the shared mapper,
          // same as every other status code here — kept in one place so any future
          // conditional-write endpoint (If-Match on delete/copy, etc.) gets it for free.
