  ...
  createContainer: projectScopedProcedure
    ...
    .mutation(async ({ input, ctx }): Promise<boolean> => {
      return withErrorHandling(async () => {
        ...
        // Swift's PUT on a container is idempotent (upsert semantics), so re-creating an
        // existing container silently succeeds instead of erroring. Check existence explicitly
        // first, mirroring the Ceph bucket pre-check (see containerRouter.ts `create`).
