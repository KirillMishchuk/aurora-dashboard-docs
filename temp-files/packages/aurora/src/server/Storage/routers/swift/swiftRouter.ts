  ...
  createContainer: projectScopedProcedure
    ...
    .mutation(async ({ input, ctx }): Promise<boolean> => {
      return withErrorHandling(async () => {
        ...
        // The probe PUT carries only X-Storage-Policy: it's the sole create-only
        // container header, safe to send unconditionally (409 on mismatch against
        // an existing container, no-op on match). Every other X-Container-* header
        // (metadata/ACL/quota/versioning/temp-url-key) would be silently applied by
        // Swift to an already-existing container on an idempotent PUT — that's the
        // metadata-leak bug this split fixes. Those are applied via a follow-up POST
        // below, only once we know a container was genuinely created (201).
    ...
    .mutation(async ({ input, ctx }): Promise<boolean> => {
      return withErrorHandling(async () => {
        ...
        const headers: Record<string, string> = {
          ...
          "Content-Length": "0",
          // Object-level PUT (unlike container-level PUT) supports an atomic
          // create-if-not-exists: Swift returns 412 Precondition Failed, and Ceph RGW
          // maps to ERR_PRECONDITION_FAILED, when the object already exists. This makes
          // the existence check server-authoritative — the client-side `existingRows`
          // check (in CreateFolderModal) remains a fast-path UX check only, not the guarantee.
          "If-None-Match": "*",
        }
        ...
        // Correct call shape: body is the 2nd argument, options (headers) the 3rd —
        // see copyObject above for the reference pattern. Passing `{ headers, body }`
        // as the 2nd argument (the previous bug) sends it as a JSON-stringified body
        // instead, so none of the headers above were ever actually sent.
