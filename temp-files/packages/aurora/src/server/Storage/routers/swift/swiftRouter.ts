  ...
  createContainer: projectScopedProcedure
    ...
    .mutation(async ({ input, ctx }): Promise<boolean> => {
      return withErrorHandling(async () => {
        ...
        // Swift (and Ceph RGW) container PUT is idempotent: it returns 201 when the
        // container is newly created and 202 when it already existed. There is no
        // conditional create-if-not-exists primitive for container PUT (If-None-Match
        // is object-level, not honoured here), so branching on the response status is
        // the only race-free way to detect a pre-existing container — a HEAD-then-PUT
        // check has a TOCTOU window where two concurrent creates can both "pass" the
        // HEAD check and both succeed silently.
        // Accepted limitation: on the 202 path, any caller-supplied metadata headers
        // above have already been applied to the pre-existing container before we
        // throw CONFLICT below. No in-repo caller currently passes such options.
