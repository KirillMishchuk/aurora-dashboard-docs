...
interface MockContextOptions {
  ...
  endpoint?: string
  region?: string
  /** Pre-aborted (or later-abortable) signal to simulate a client navigating away mid-scan. */
...
export const createMockContext = (options: MockContextOptions = {}): MockContext => {
  ...
  return {
    // Prod exposes the abort signal both as `ctx.signal` and on the Fastify request
    // (`context.ts:150`); Storage routers read `ctx.req.signal`, so the mock must carry both.
    req: { headers: {}, signal },
