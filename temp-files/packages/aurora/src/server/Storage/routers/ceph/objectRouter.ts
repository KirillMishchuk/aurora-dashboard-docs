...
export const objectRouter = {
  ...
  createFolder: cephProtectedProcedure
    ...
    .mutation(async ({ ctx, input }): Promise<boolean> => {
      ...
      try {
        // IfNoneMatch: "*" makes this an atomic create-if-not-exists — S3/RGW
        // rejects with PreconditionFailed (412) if the key already exists,
        // so the existence check is server-authoritative rather than relying
        // solely on the client's (paginated, racy) existingRows check.
        await s3.send(
