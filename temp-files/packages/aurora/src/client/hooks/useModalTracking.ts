...
export const useModalTracking = ({ isOpen, actionPrefix }: UseModalTrackingOptions): UseModalTrackingReturn => {
   ...
   */
  // Stable identities (useCallback) matter here: consumers pass these into their own
  // useEffect dependency arrays (e.g. to reset a mutation on close), and an unmemoized
  // function recreated every render would make such an effect re-fire every render too.
