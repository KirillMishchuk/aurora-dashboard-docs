...
interface CreateBucketModalProps {
  ...
  /** Fires only for failures reported after the modal has closed (e.g. post-create follow-up steps), not for in-modal validation/create failures. */
...
// `onError` is retained in the props interface for API stability (kept in sync with
// CreateContainerModal, which still uses it for post-close partial-success failures) but is
// intentionally not consumed here — non-CONFLICT create failures are now surfaced in-modal via
// the `submitError` banner below rather than through this callback.
export const CreateBucketModal = ({ isOpen, onClose, onSuccess, existingBuckets = [] }: CreateBucketModalProps) => {
  ...
  const createBucketMutation = trpcReact.storage.ceph.containers.create.useMutation({
    ...
    onError: (error) => {
      ...
      // The modal stays open on any failure (no handleClose here), so undo markSubmitted's
