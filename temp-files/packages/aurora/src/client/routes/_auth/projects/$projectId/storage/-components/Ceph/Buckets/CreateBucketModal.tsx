...
export const CreateBucketModal = ({
  ...
  const createBucketMutation = trpcReact.storage.ceph.containers.create.useMutation({
    ...
    onError: (error) => {
      ...
      // A conflict means the name is taken — keep the modal open and show the inline error
      // this issue asks for instead of the generic post-submit toast. Any other failure
      // (permissions, network, ...) falls back to the existing toast-and-close behavior.
