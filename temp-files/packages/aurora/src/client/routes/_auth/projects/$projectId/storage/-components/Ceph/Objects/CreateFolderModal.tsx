...
export function CreateFolderModal({ bucketName, currentPrefix, isOpen, onClose, onSuccess }: CreateFolderModalProps) {
  ...
  const handleFolderNameChange = (value: string) => {
    setFolderName(value)
    // Validation only runs eagerly once an error is already showing, so it
    // can clear as the user fixes it — the first check happens on submit,
    // not on every keystroke (matches the Swift CreateFolderModal).
    if (validationError) {
