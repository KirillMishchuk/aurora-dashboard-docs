...
4. **Save the report to the review folder.** After presenting the combined
   findings to the user, automatically save a comprehensive markdown document
   to `<repo-parent>/DOCS/review/YYYY-MM-DD-<branch-name>-triple-review.md`:
   - Use current date in ISO format (YYYY-MM-DD)
   - Extract branch name from `git branch --show-current`
   - Create the `<repo-parent>/DOCS/review/` directory if it doesn't exist
   - Include all findings with full details: severity, location, issue
     description, recommendations, code examples, and effort estimates
   - Add sections for: Executive Summary, High/Medium/Low Severity Findings,
     Positive Observations, Priority Recommendations, Impact Assessment,
     Files Reviewed, and Conclusion
   - Mention to the user that the report has been saved and provide the path
