...
2. **Plan.** Launch the `dev-planner` agent (foreground) with the task
   ...
   BFF, React 19 client) so it grounds itself via `CLAUDE.md` and the KB per
   ...
   from the KB README, resolved the same way step 3 resolves `PLANS_DIR`
   (`KB_README="$(dirname "$(git rev-parse --show-toplevel)")/DOCS/aurora-dashboard-kb/README.md"`),
   and run
